...
class _DataManagementScreenState extends State<DataManagementScreen> {
  ...
  void _message() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(text), backgroundColor: vm.error == null ? null : Colors.red),
    );
  }

  Future<void> _run(Future<void> Function(IntegrationViewModel vm) action,
  ...
  Future<String?> _xlsxPath(String name) async {
    ...
    return FilePicker.platform.saveFile(
      dialogTitle: '저장 위치 선택', fileName: name,
            ...
            child: ConstrainedBox(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                ...
                if (vm.isSyncingFull) ...[
                  ...
                  LinearProgressIndicator(value: vm.syncTotalTargets == 0 ? null : vm.syncCompletedTargets / vm.syncTotalTargets),
                  const SizedBox(height: 6),
                  Text('동기화 ${vm.syncCompletedTargets}/${vm.syncTotalTargets}'),
                ],
                const SizedBox(height: 16),
                _Grid(wide: wide, children: [
                  _Group('시스템 간 동기화', Icons.sync_alt_outlined, [
                    _Cmd('전체 VOC · 매뉴얼 공유', Icons.cloud_upload_outlined,
                        () => _run((v) => v.forwardFullVocAndManualToPeerApps())),
                    _Cmd('상대 앱 VOC 가져오기', Icons.cloud_download_outlined,
                        () => _run((v) async { await v.pullVocFromPeerApps(); }, refresh: true)),
                    _Cmd('초기 핸드셰이크', Icons.handshake_outlined,
                        () => _run((v) async { await v.bootstrapFromPeerApps(); }, refresh: true)),
                    _Cmd('실패 동기화 재시도 (${vm.syncRetryQueueCount})', Icons.refresh_outlined,
                        vm.syncRetryQueueCount == 0 ? null : () => _run((v) => v.retryPendingSyncQueue())),
                  ]),
                  _Group('가져오기 · 백업', Icons.folder_copy_outlined, [
                    _Cmd('Outlook 메일에서 VOC 수집', Icons.mark_email_read_outlined,
                        () => _run((v) async { await v.collectOutlookAndCreateVoc(top: 20); }, refresh: true)),
                    _Cmd('VOC 가져오기', Icons.file_upload_outlined, _importVoc),
                    _Cmd('VOC 내보내기', Icons.file_download_outlined, () => _export(false)),
                    _Cmd('VOC 입력 템플릿', Icons.description_outlined, () => _export(true)),
                  ]),
                  _Group('AI · 검색 데이터', Icons.auto_awesome_motion_outlined, [
                    _Cmd('Vector DB 재생성', Icons.hub_outlined, () => _run((v) async { await v.rebuildVectorDb(); })),
                    _Cmd('AI 캐시 초기화', Icons.cleaning_services_outlined, () async {
                  ...
                  _Group('초기화', Icons.warning_amber_rounded, [
                    _Cmd('VOC 데이터 전체 초기화', Icons.delete_forever_outlined, () async {
                      if (await _confirm('VOC 데이터 전체 초기화', 'VOC, 답변, 지식베이스, Vector DB와 AI 캐시가 삭제됩니다. 되돌릴 수 없습니다.', danger: true)) {
                ...
                if (vm.syncRuntimeLogs.isNotEmpty) ...[
                  ...
                  Card(child: ExpansionTile(
                    ...
                    children: [SizedBox(
                      ...
                      height: 180,
                      child: SingleChildScrollView(child: SelectableText(vm.syncRuntimeLogs.join('\n'))),
                    )],
                  )),
                ],
              ]),
...
class _Header extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Card(child: Padding(
      ...
      child: Row(children: [
        CircleAvatar(backgroundColor: tone.withValues(alpha: .12), child: Icon(Icons.storage_outlined, color: tone)),
        const SizedBox(width: 14),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('데이터 및 동기화 운영', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w800)),
          const SizedBox(height: 4),
          Text('가져오기, 백업, 동기화와 초기화 같은 실행 작업을 관리합니다.', style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: cs.onSurfaceVariant)),
        ])),
        Text(active ? '수신기 실행 중' : '수신기 확인 필요', style: TextStyle(color: tone, fontWeight: FontWeight.w700)),
      ]),
    ));
  }
}
...
class _Grid extends StatelessWidget {
  ...
  final bool wide; final List<Widget> children;
  @override
  Widget build(BuildContext context) => LayoutBuilder(builder: (context, c) {
    final w = wide ? (c.maxWidth - 14) / 2 : c.maxWidth;
    return Wrap(spacing: 14, runSpacing: 14, children: children.map((e) => SizedBox(width: w, child: e)).toList());
  });
}

class _Group extends StatelessWidget {
  const _Group(this.title, this.icon, this.commands, {this.danger = false});
  final String title; final IconData icon; final List<_Cmd> commands; final bool danger;
  @override
  Widget build(BuildContext context) => Card(child: Padding(
    padding: const EdgeInsets.all(18),
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [Icon(icon, color: danger ? Colors.red : Theme.of(context).colorScheme.primary), const SizedBox(width: 10), Expanded(child: Text(title, style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w800)))]),
      const SizedBox(height: 14),
      ...commands.map((c) => Padding(padding: const EdgeInsets.only(bottom: 8), child: SizedBox(width: double.infinity, child: OutlinedButton.icon(
        style: c.danger ? OutlinedButton.styleFrom(foregroundColor: Colors.red) : null,
        onPressed: c.action, icon: Icon(c.icon), label: Text(c.label),
      )))),
    ]),
  ));
}
...
class _Cmd {
  ...
  final String label; final IconData icon; final VoidCallback? action; final bool danger;
}
