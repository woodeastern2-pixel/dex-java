...
# Exercise the safe horizontal swipe path with scrollbar chrome disabled.
adb shell input swipe 960 720 430 720 350 || true
sleep 0.4

# Capture the redesigned quick-entry cards first, then the photo area for the existing photo regression.
start_capture RecordActivity '빠른 선택' '04-record-additional.png' --el senior_id 4 --ei visual_step 2 --ez visual_photos true
...
echo 'v0.4.8 final validation passed: share regression + dashboard counts + horizontal choices + redesigned additional cards + rendered screens.'
