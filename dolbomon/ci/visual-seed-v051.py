...
# Keep screenshot validation deterministic. The production app still uses AdMob/UMP,
# but consent/ad overlays must not cover the app UI while CI is validating rendered screens.
for activity in ['MainActivity.java', 'HistoryActivity.java', 'MessageCenterActivity.java', 'SettingsActivity.java']:
    path = root / 'app/src/main/java/com/easternwood/dolbomon' / activity
    text = path.read_text()
    text = text.replace(
        'AdsManager.initialize(this, () -> AdsManager.attachBanner(this, adHolder));',
        '// Ads/UMP intentionally disabled only in CI visual-validation source.'
    )
